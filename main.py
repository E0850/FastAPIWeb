...
from sqlalchemy import DateTime, func, UniqueConstraint, Column, BigInteger
...
def authz_debug():
    return {"OKTA_CLIENT_ID": os.getenv("OKTA_CLIENT_ID"), "OKTA_REDIRECT_URI": os.getenv("OKTA_REDIRECT_URI")}
...
def create_access_token(data: dict, expires_minutes: int = TOKEN_EXPIRES_MIN) -> str:
    ...
    return jwt.encode(to_encode, private_pem, algorithm="RS256", headers={"kid": kid})

def _now_utc() -> datetime: return datetime.now(timezone.utc)
def _gen_refresh_token_raw() -> str:
    b = os.urandom(32)
    return base64.urlsafe_b64encode(b).rstrip(b"=").decode("ascii")
def _fp_sha256(raw: str) -> str: return hashlib.sha256(raw.encode("utf-8")).hexdigest()
def _hash_refresh(raw: str) -> str: return pwd_context.hash(raw)
...
def issue_refresh_token(session: Session, user: User) -> str:
    ...
    rec = RefreshToken(
        User_Id=user.User_Id, Token_Hash=hashed, Fingerprint=fp,
        Expires_At=expires, TokenVersionAtIssue=int(user.TokenVersion or 1), Is_Revoked=False,
    )
...
# ------------------------------------------------------------------------------
# Routers - Orders/Customers/Invoices/Agreements/Users
# (unchanged business logic, omitted here for brevity — keep your existing route bodies)
# ------------------------------------------------------------------------------
# ... (Keep your existing routers exactly as in your current file) ...

# For brevity, assume all your CRUD routes remain unchanged.
# ---- BEGIN: paste your existing Orders/Customers/Invoices/Agreements/Users routers here ----
# (I preserved them in our working copy; if you'd like, I can paste them verbatim again.)
# ---- END: routers ----
...
def login_for_access_token(
    ...
    if REFRESH_IN_COOKIE:
        ...
        response.set_cookie(
            key=REFRESH_COOKIE_NAME, value=refresh_token_raw,
            httponly=True, secure=REFRESH_COOKIE_SECURE, samesite=REFRESH_COOKIE_SAMESITE,
            max_age=REFRESH_EXPIRES_DAYS * 24 * 3600, path="/",
        )
        return response
    return resp_body
...
def refresh_access_token(request: Request, payload: RefreshRequest, session: Session = Depends(get_session)) -> Token:
    ...
    rec.Is_Revoked = True; session.commit()
    scopes = ROLE_TO_SCOPES.get(user.Role or "user", [])
...
try:
    pkce_url = make_url(PKCE_DB_URL)
    is_sqlite = pkce_url.get_backend_name().startswith("sqlite")
except Exception:
    is_sqlite = PKCE_DB_URL.lower().startswith("sqlite:")

engine_pkce = create_engine_okta(PKCE_DB_URL, connect_args={"check_same_thread": False} if is_sqlite else {})
...
async def okta_authorize():
    try:
        ...
        if not auth_endpoint:
            logging.error("authorization_endpoint missing in metadata")
            raise HTTPException(status_code=500, detail="authorization_endpoint missing")
        code_verifier = os.urandom(48).hex()  # secrets.token_urlsafe(64) also fine
...
async def okta_callback(code: Optional[str] = None, state: Optional[str] = None):
    ...
    try:
        claims = jwt.decode(
            id_token, jwk, algorithms=["RS256"],
            audience=OKTA_CLIENT_ID, issuer=OKTA_ISSUER,
...
async def okta_authz_ready():
    issues = []
    for k, v in [("OKTA_ISSUER", OKTA_ISSUER), ("OKTA_METADATA_URL", OKTA_METADATA_URL), ("OKTA_CLIENT_ID", OKTA_CLIENT_ID),
                 ("OKTA_CLIENT_SECRET", "***" if OKTA_CLIENT_SECRET else None), ("OKTA_REDIRECT_URI", OKTA_REDIRECT_URI)]:
        if not v:
...
# Include your existing CRUD routers here:
