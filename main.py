...
for r in customers_router.routes:
    if r.path in ("/CreateCustomers", "/UpdateCustomers/{Customer_Number}", "/DeleteCustomers/{Customer_Number}"):
        r.dependencies = [WRITE_DEPS["customers"]]
for r in invoices_router.routes:
    if r.path in ("/CreateInvoices", "/UpdateInvoices/{Invoice_Number}", "/DeleteInvoices/{Invoice_Number}"):
        r.dependencies = [WRITE_DEPS["invoices"]]
for r in agreements_router.routes:
    if r.path in ("/CreateAgreements", "/UpdateAgreements/{Agreement_number}", "/DeleteAgreement/{Agreement_number}"):
        r.dependencies = [WRITE_DEPS["agreements"]]
for r in users_router.routes:
    if r.path in ("/CreateUsers", "/UpdateUsers/{email}", "/UpdatePassword/{email}", "/AssignRole/{email}", "/DeleteUsers/{email}", "/RevokeTokens/{email}"):
        r.dependencies = [WRITE_DEPS["users"]]
