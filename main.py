    ...
    def jwks(self) -> dict:
        ...
        for kid, pub_pem in self.pub_by_kid.items():
            try:
                ...
                keys.append({"kty": "RSA", "kid": kid, "alg": JWT_RS256_ALG, "use": "sig", "n": _b64url(n), "e": _b64url(e)})
...
def order_out(o: Order) -> OrderOut:
    return OrderOut(Order_Number=o.Order_Number, Customer_Number=o.Customer_Number, Quantity=o.Quantity, Price=o.Price)
...
def customer_out(c: Customer) -> CustomerOut:
    return CustomerOut(Customer_Number=c.Customer_Number, Customer_Name=c.Customer_Name, Customer_Address=c.Customer_Address,
                       Contact_Number=c.Contact_Number, Email_Address=c.Email_Address)
...
def invoice_out(i: Invoice) -> InvoiceOut:
    return InvoiceOut(Invoice_Number=i.Invoice_Number, Order_Number=i.Order_Number, Invoice_Date=i.Invoice_Date,
                      Invoice_Email=i.Invoice_Email, Amount=i.Amount, Customer_Number=i.Customer_Number)
...
def issue_refresh_token(session: Session, user: User) -> str:
    # Revoke any existing active tokens first
    ...
    rec = RefreshToken(User_Id=user.User_Id, Token_Hash=hashed, Fingerprint=fp, Expires_At=expires,
                       TokenVersionAtIssue=int(user.TokenVersion or 1), Is_Revoked=False)
    session.add(rec)
    session.commit()
    return raw
...
# ====================== Current User & Scope checks ======================
class Principal(BaseModel):
    email: str
    scopes: List[str]

async def _principal_from_request(request: Request, session: Session) -> Principal:
    ...
    if not user or not user.Is_Active or token_ver != user.TokenVersion:
        raise HTTPException(status_code=401, detail="Token revoked or user inactive")
    # attach identity to request for per-user rate limiting
...
async def get_current_user(request: Request, session: Session = Depends(get_session)) -> User:
    ...
    # stash scopes for other deps
    user.Scopes = p.scopes
    return user
...
# Role & scope enforcement (local implementation)
...
def search_customer(request: Request, SQRY: Optional[str] = Query(None), limit: int = Query(50, ge=1, le=500), offset: int = Query(0, ge=0), session: Session = Depends(get_session)) -> List[CustomerOut]:
    ...
    if SQRY:
        search_term = f"%{SQRY}%"
        stmt = stmt.where(or_(cast(Customer.Customer_Number, String).ilike(search_term), cast(Customer.Customer_Name, String).ilike(search_term), cast(Customer.Customer_Address, String).ilike(search_term), cast(Customer.Contact_Number, String).ilike(search_term), cast(Customer.Email_Address, String).ilike(search_term)))
    stmt = stmt.order_by(Customer.Customer_Number).limit(limit).offset(offset)
    results = session.scalars(stmt).all()
...
@limiter.limit("50/minute")
def create_customer(request: Request, payload: List[CustomerIn], session: Session = Depends(get_session), _: User = Depends(require_scopes(["customers:write"]))):
    created = []
    for item in payload:
...
def search_invoice(request: Request, SQRY: Optional[str] = Query(None), limit: int = Query(50, ge=1, le=500), offset: int = Query(0, ge=0), session: Session = Depends(get_session)) -> List[InvoiceOut]:
    ...
    if SQRY:
        search_term = f"%{SQRY}%"
        stmt = stmt.where(or_(cast(Invoice.Invoice_Number, String).ilike(search_term), cast(Invoice.Customer_Number, String).ilike(search_term), cast(Invoice.Order_Number, String).ilike(search_term), cast(Invoice.Invoice_Date, String).ilike(search_term), cast(Invoice.Invoice_Email, String).ilike(search_term), cast(Invoice.Amount, String).ilike(search_term)))
    stmt = stmt.order_by(Invoice.Invoice_Number).limit(limit).offset(offset)
    results = session.scalars(stmt).all()
...
@limiter.limit("50/minute")
def create_invoices(request: Request, payload: List[InvoiceIn], session: Session = Depends(get_session), _: User = Depends(require_scopes(["invoices:write"]))):
    created = []
    for item in payload:
...
def search_agreements(request: Request, SQRY: Optional[str] = Query(None), limit: int = Query(50, ge=1, le=500), offset: int = Query(0, ge=0), session: Session = Depends(get_session)) -> List[AgreementOut]:
    ...
    if SQRY:
        search_term = f"%{SQRY}%"
        stmt = stmt.where(or_(Agreement.Agreement_number.ilike(search_term), cast(Agreement.Customer_Number, String).ilike(search_term), cast(Agreement.Customer_site, String).ilike(search_term), cast(Agreement.Your_reference_1, String).ilike(search_term), cast(Agreement.Telephone_number_1, String).ilike(search_term), cast(Agreement.customers_order_number, String).ilike(search_term), cast(Agreement.Agreement_order_type, String).ilike(search_term), Agreement.Termination_date.ilike(search_term), cast(Agreement.Line_charge_model, String).ilike(search_term), Agreement.Address_line_1.ilike(search_term), Agreement.Address_line_2.ilike(search_term), Agreement.Address_line_3.ilike(search_term), Agreement.Address_line_4.ilike(search_term), Agreement.Salesperson.ilike(search_term), cast(Agreement.Minimum_rental_type, String).ilike(search_term), cast(Agreement.Minimum_order_value, String).ilike(search_term), Agreement.Currency.ilike(search_term), Agreement.Reason_code_created_agreement.ilike(search_term), Agreement.User.ilike(search_term), cast(Agreement.Minimum_hire_period, String).ilike(search_term), Agreement.Payment_terms.ilike(search_term), Agreement.Price_list.ilike(search_term), Agreement.Reason_code_terminated_agreement.ilike(search_term), Agreement.Project_number.ilike(search_term)))
    stmt = stmt.order_by(Agreement.Agreement_number).limit(limit).offset(offset)
    results = session.scalars(stmt).all()
...
@limiter.limit("50/minute")
def create_agreement(request: Request, payload: List[AgreementIn], session: Session = Depends(get_session), _: User = Depends(require_scopes(["agreements:write"]))):
    created = []
    for item in payload:
    ...
    try:
        ...
        session.rollback()
        logging.exception("Agreement create failed")
...
def search_user(request: Request, SQRY: Optional[str] = Query(None), limit: int = Query(50, ge=1, le=500), offset: int = Query(0, ge=0), session: Session = Depends(get_session)) -> List[UserPublic]:
    ...
    if SQRY:
        search_term = f"%{SQRY}%"
        stmt = stmt.where(or_(cast(User.Email_Address, String).ilike(search_term), cast(User.User_Name, String).ilike(search_term), cast(User.Location_Address, String).ilike(search_term), cast(User.Contact_Number, String).ilike(search_term), cast(User.Vat_Number, String).ilike(search_term), cast(User.Hashed_Pword, String).ilike(search_term), cast(User.Role, String).ilike(search_term)))
    stmt = stmt.order_by(User.Email_Address).limit(limit).offset(offset)
    results = session.scalars(stmt).all()
...
def create_user(request: Request, payload: UserCreate, session: Session = Depends(get_session), _: User = Depends(require_scopes(["users:manage"])) ) -> UserPublic:
    ...
    if session.scalar(select(User).where(User.Email_Address == email)):
        raise HTTPException(status_code=409, detail="Email already registered")
    hashed = get_password_hash(data.pop("Password"))
    u = User(User_Name=data["User_Name"], Location_Address=data.get("Location_Address"), Email_Address=email, Contact_Number=data.get("Contact_Number"), Vat_Number=data.get("Vat_Number"), Hashed_Pword=hashed)
    session.add(u)
    session.commit()
    session.refresh(u)
    return user_out(u)
...
def update_user(request: Request, email: str, payload: UserUpdate, session: Session = Depends(get_session), _: User = Depends(require_scopes(["users:manage"])) ) -> UserPublic:
    ...
    if payload.User_Name is not None: u.User_Name = payload.User_Name
    if payload.Location_Address is not None: u.Location_Address = payload.Location_Address
    if payload.Contact_Number is not None: u.Contact_Number = payload.Contact_Number
    if payload.Vat_Number is not None: u.Vat_Number = payload.Vat_Number
...
def update_password(email: str, payload: UserPasswordUpdate, session: Session = Depends(get_session), _: User = Depends(require_scopes(["users:manage"])) ):
    u = session.scalar(select(User).where(User.Email_Address == email.strip().lower()))
    if not u:
        raise HTTPException(status_code=404, detail="User not found")
...
@limiter.limit("50/minute")
def revoke_tokens_by_email(request: Request, email: str, session: Session = Depends(get_session), _: User = Depends(require_scopes(["users:manage"])) ):
    u = session.scalar(select(User).where(User.Email_Address == email.strip().lower()))
    if not u:
        raise HTTPException(status_code=404, detail="User not found")
    u.TokenVersion = (u.TokenVersion or 1) + 1
...
def login_for_access_token(request: Request, form_data = Depends(OAuth2PasswordRequestForm), session: Session = Depends(get_session)) -> Token:
    ...
    if REFRESH_IN_COOKIE:
        ...
        response.set_cookie(key=REFRESH_COOKIE_NAME, value=refresh_token_raw, httponly=True, secure=SECURE_COOKIES, samesite=REFRESH_COOKIE_SAMESITE, max_age=REFRESH_EXPIRES_DAYS * 24 * 3600, path="/")
        return response
    return resp_body

@auth_router.get("/me", response_model=UserPublic, summary="Identify me! (Requires token)")
...
def register_user(payload: UserCreate, session: Session = Depends(get_session)) -> UserPublic:
    ...
    u = User(User_Name=payload.User_Name, Location_Address=payload.Location_Address, Email_Address=email, Contact_Number=payload.Contact_Number, Vat_Number=payload.Vat_Number, Hashed_Pword=hashed)
    session.add(u)
    session.commit()
    session.refresh(u)
    return user_out(u)
...
def refresh_access_token(request: Request, payload: RefreshRequest, session: Session = Depends(get_session)) -> Token:
    ...
    if REFRESH_IN_COOKIE:
        ...
        response.set_cookie(key=REFRESH_COOKIE_NAME, value=new_refresh, httponly=True, secure=SECURE_COOKIES, samesite=REFRESH_COOKIE_SAMESITE, max_age=REFRESH_EXPIRES_DAYS * 24 * 3600, path="/")
        return response
    return body
...
engine_pkce = (create_engine_okta(PKCE_DB_URL, connect_args={"check_same_thread": False}) if PKCE_DB_URL.strip().lower().startswith('sqlite') else create_engine_okta(PKCE_DB_URL))
...
async def okta_callback(code: Optional[str] = None, state: Optional[str] = None):
    ...
    # Build base body
    data = {"grant_type": "authorization_code", "code": code, "redirect_uri": OKTA_REDIRECT_URI, "code_verifier": code_verifier}
    ...
    try:
        ...
        claims = jwt.decode(id_token, key, algorithms=["RS256"], audience=OKTA_CLIENT_ID, issuer=OKTA_ISSUER, access_token=okta_access, options={"require_exp": True, "require_iat": True, "require_aud": True, "require_iss": True})
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"ID token verification failed: {str(e)}")
    if claims.get("nonce") != expected_nonce:
        raise HTTPException(status_code=400, detail="Nonce mismatch")
    # Provision/lookup local user and mint first-party API token
    ...
    with Session(engine) as s:
        ...
        api_access = jwt.encode({"sub": user.Email_Address, "ver": user.TokenVersion, "scopes": scopes, "exp": datetime.now(timezone.utc) + timedelta(minutes=TOKEN_EXPIRES_MIN)}, priv, algorithm=JWT_RS256_ALG, headers={"kid": kid_active})
    response = RedirectResponse(url=POST_LOGIN_REDIRECT, status_code=302)
    response.set_cookie(key="api_access_token", value=api_access, httponly=True, secure=SECURE_COOKIES, samesite=COOKIE_SAMESITE, max_age=TOKEN_EXPIRES_MIN * 60, path="/")
    if tokens.get("access_token"):
        response.set_cookie(key="okta_access_token", value=tokens["access_token"], httponly=True, secure=SECURE_COOKIES, samesite=COOKIE_SAMESITE, max_age=tokens.get("expires_in", TOKEN_EXPIRES_MIN * 60), path="/")
    if tokens.get("refresh_token"):
        response.set_cookie(key="okta_refresh_token", value=tokens["refresh_token"], httponly=True, secure=SECURE_COOKIES, samesite=COOKIE_SAMESITE, max_age=30 * 24 * 3600, path="/")
    return response
...
# Entrypoint (optional when running as module on Render)
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("main_combined:app", host="127.0.0.1", port=8000, reload=True)
